...
class ByteCodeParser(factory: BCF) extends BCP {
  ...
  def parse(bytes: Vector[Byte]): Vector[ByteCode] = createBytecodes(bytes)._2
...
def createBytecodes(bytes: Vector[Byte], bytecodes: Vector[ByteCode] = Vector.empty): (Vector[Byte], Vector[ByteCode]) = bytes match {
    ...
    // of the tail (tt)
    case Iconst +: ht +: tt => createBytecodes(tt, bytecodes :+ factory.make(Iconst, ht))
    ...
    case _+:_ | _+:_+:_ => createBytecodes(bytes.tail, bytecodes :+ factory.make(bytes.head))
